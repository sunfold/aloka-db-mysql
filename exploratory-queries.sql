...
-- Determine how many states have no active customers. 
-- Answer: 31
...
-- Determine which state has the highest count of active customers. 
-- Answer: CA with 6 customers.
SELECT 
	state, 
...
SELECT 
	RANK() OVER(ORDER BY avg_per_state DESC) AS ranking,
    state,
    avg_per_state
FROM savg
ORDER BY avg_per_state DESC
...
-- Order by average length of stay descending.
-- Answer (in terms of average stay): 2022 = 11.00 days, 2024 = 10.54 days, 2023 = 9.63 days
WITH ddiff AS (
	SELECT 
		EXTRACT(YEAR FROM check_in_date) AS years,
...
-- Find the total gross revenue per month, per year, for 2023 and 2024.
-- Compare to overall average gross revenue
-- Order by highest gross revenue descending
-- Which month had the highest gross revenue, and what is the overall average gross revenue per month?
-- Answer, part 1: February 2024 had the highest monthly gross revenue at $11,726.14. 
-- Answer, part 2: The overall average gross revenue per month is $6558.01.
WITH dtcte as (
SELECT
...
-- Determine gross revenue for each hotel. Order by total gross revenue ascending.
-- Which hotel has had the lowest gross revenue?
